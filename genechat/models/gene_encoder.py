...
from builtins import list, dict, int
...
class GPT2Embeddings(nn.Module):
    """
    A backbone model consists of a stack of blocks. If you use attention, then positional embeddings are included. When using Hyena, then the pos emb revert to doing nothing.
    """
...
class HyenaDNAModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_layer: int, d_inner: int, vocab_size: int,
                 ...
                 device=None, dtype=None, mode='pool', **kwargs) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super().__init__()
        ...
        if self.use_head:
            self.head = SequenceDecoder(d_model=d_model, d_output=None, l_output=0,mode=mode)
...
class HyenaDNAPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls,
        ...
        #print(pretrained_model_name_or_path, device)
        ...
        print("Loaded pretrained weights ok!", next(scratch_model.parameters()).device)
